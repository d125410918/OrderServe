...
export function buildCheckoutLines(cartItems: CartLine[], participants: RoomParticipant[]): CheckoutLine[] {
  const submittedRoomLines = participants.filter((participant) => participant.status === "submitted")
