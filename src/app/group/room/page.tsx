...
export default function GroupRoomPage() {
  ...
  const { state, createRoom, submitParticipant, extendRoom, setRoomState, clearCart } = useOrder();
  const [remaining, setRemaining] = useState("15:00");
  useEffect(() => {
    if (!state.room) createRoom({ code: "482761", hostName: "小美", deadlineAt: Date.now() + 15 * 60_000, branchId: state.branchId, mode: state.mode });
  }, [state.room, createRoom, state.branchId, state.mode]);
  useEffect(() => {
  ...
  useEffect(() => {
    if (!state.room) return;
    const timer = window.setInterval(() => setRemaining(formatRemaining(state.room!.deadlineAt)), 1000);
